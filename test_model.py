...
def get_args():
    ...
    parser.add_argument("--image", "-i", type=str, default='test3.png')
...
for b,l in zip(final_boxes, final_labels):
    ...
    if l == 1:
        ...
        (w, h), _ = cv2.getTextSize(classes[l], cv2.FONT_HERSHEY_SIMPLEX, 0.6, 1) 
        image = cv2.rectangle(image, (xmin, ymin - 20), (xmin + w, ymin), (0, 255, 0), -1)
        ...
        image = cv2.rectangle(image, (xmin, ymin - 20), (xmin + w, ymin), (255, 0, 0), -1)
        image = cv2.putText(image, classes[l], (xmin, ymin - 5)   , cv2.FONT_HERSHEY_SIMPLEX, 
                0.6, (255,255,255), 1, cv2.LINE_AA)
